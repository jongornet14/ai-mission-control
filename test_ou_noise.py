...
def test_ou_noise():
    ...
    ou_noise = OrnsteinUhlenbeckNoise(size=size, mu=mu, theta=theta, sigma=sigma, dt=dt)
    ...
    test_params = [
        {
            "theta": 0.05,
            "sigma": 0.1,
            "description": "Low mean reversion, low volatility",
        },
        {
            "theta": 0.5,
            "sigma": 0.1,
            "description": "High mean reversion, low volatility",
        },
        {
            "theta": 0.15,
            "sigma": 0.5,
            "description": "Medium mean reversion, high volatility",
        },
    ]

    for params in test_params:
        ou_test = OrnsteinUhlenbeckNoise(
            size=1, mu=0.0, theta=params["theta"], sigma=params["sigma"], dt=0.01
        )
        ...
        print(
            f"  {params['description']}: mean={test_mean:.3f}, std={test_std:.3f}, theoretical_std={theoretical_std:.3f}"
        )
    ...
    try:
        ...
        for i, config in enumerate(configs):
            ...
            ou = OrnsteinUhlenbeckNoise(
                size=1, mu=0.0, theta=config["theta"], sigma=config["sigma"], dt=0.01
            )
