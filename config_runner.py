...
def list_configs(config_dir: str = "configs"):
    ...
    for config_file in sorted(configs):
        ...
        print(
            colored(
                f"   Algorithm: {algo} | Environment: {env} | Episodes: {episodes}",
                "yellow",
            )
        )
        print()
...
Examples:
  ...
  python config_runner.py --config configs/sample_ddpg_config.json --worker_id 1 --device cpu
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available config files"
    )
    parser.add_argument("--validate", type=str, help="Validate specific config file")
    parser.add_argument(
        "--validate-all", action="store_true", help="Validate all config files"
    )
