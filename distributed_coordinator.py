...
class DistributedCoordinator:
    def __init__(self, shared_dir, num_workers=4, check_interval=30, min_episodes=50):
        ...
        for dir_path in [self.models_dir, self.metrics_dir, self.best_model_dir, self.coordinator_dir]:
    ...
    def find_best_worker(self) -> Optional[int]:
        ...
        for worker_id, metrics in all_metrics.items():
            ...
            if metrics.get('total_episodes', 0) < self.min_episodes:
                print(f"Worker {worker_id}: Insufficient episodes ({metrics.get('total_episodes', 0)}/{self.min_episodes})")
                continue
    ...
    def copy_best_model(self, best_worker_id: int) -> bool:
        ...
        try:
            ...
            model_files = list(self.models_dir.glob(f"worker_{best_worker_id}_episode_*.pt"))
            if not model_files:
            ...
            print(f"   Performance: {best_metrics.get('reward_change', 0):.4f} reward change")
    ...
    def save_coordination_log(self):
        ...
        try:
            log_data = {
                ...
                'best_worker_history': [entry for entry in self.coordination_log if 'best_worker' in entry]
            }
    ...
    def perform_sync(self) -> bool:
        ...
        log_entry = {
            ...
            'best_performance': best_metrics.get('reward_change', 0) if best_metrics else 0,
    ...
    def run(self, max_syncs: Optional[int] = None, max_runtime_hours: Optional[float] = None):
        """
        ...
        print(f"Max runtime: {max_runtime_hours if max_runtime_hours else 'unlimited'} hours")
            ...
            while True:
                ...
                if max_runtime_hours:
                    ...
                    if runtime_hours >= max_runtime_hours:
                        print(f"⏰ Runtime limit reached ({runtime_hours:.1f}/{max_runtime_hours} hours)")
                        break
    ...
    def cleanup(self):
        ...
        summary = {
            ...
            'coordination_efficiency': len(self.coordination_log) / max(1, self.sync_count)
        }
...
def main():
    ...
    parser.add_argument('--num_workers', type=int, default=4,
                       help='Number of workers to coordinate')
    parser.add_argument('--check_interval', type=int, default=30,
                       help='Seconds between worker checks')
    parser.add_argument('--min_episodes', type=int, default=50,
                       help='Minimum episodes before considering worker')
    parser.add_argument('--max_syncs', type=int, default=None,
                       help='Maximum number of sync cycles')
