...
class PPOAlgorithm:
    ...
    def train_step(self, rollout_data, replay_buffer=None):
        ...
        if not rollout_data:
            return {'loss_total': 0.0, 'loss_policy': 0.0, 'loss_value': 0.0, 'loss_entropy': 0.0}
        ...
        if self.discrete:
            actions = torch.tensor([step[1] for step in rollout_data], dtype=torch.long).to(self.device)
        else:
        ...
        next_observations = torch.stack([step[3] for step in rollout_data]).to(self.device)
        ...
        with torch.no_grad():
            ...
            if self.discrete:
                ...
                old_log_probs = F.log_softmax(old_logits, dim=-1).gather(1, actions.unsqueeze(1)).squeeze()
            else:
        ...
        return {
            'loss_total': (total_policy_loss + total_value_loss + total_entropy_loss) / self.num_epochs,
