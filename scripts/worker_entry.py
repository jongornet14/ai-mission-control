...
def load_worker_config(worker_id: int, shared_dir: str) -> ConfigLoader:
    ...
    """
    config_file = (
        Path(shared_dir) / "worker_configs" / f"worker_{worker_id}_config.json"
    )
    ...
    print(colored(f"Loaded worker {worker_id} config from: {config_file}", "green"))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Distributed RL Worker with JSON Config"
    )
    ...
    parser.add_argument(
        "--sync_frequency",
        type=int,
        help="Episodes between sync checks (overrides config)",
    )
    ...
    try:
        ...
        sync_frequency = args.sync_frequency or config.get(
            "distributed.sync_frequency", 10
        )
        ...
        print(colored(f"Starting Distributed Worker {args.worker_id}", "green"))
        print(colored(f"Shared directory: {args.shared_dir}", "cyan"))
        print(colored(f"Environment: {env_name}", "blue"))
        print(colored(f"Max episodes: {max_episodes}", "yellow"))
        print(colored(f"Learning rate: {lr}", "yellow"))
        print(colored(f"Sync frequency: every {sync_frequency} episodes", "cyan"))
        ...
        worker.run_distributed(
            env_name=env_name, device=device, lr=lr, sync_check_frequency=sync_frequency
        )

    except FileNotFoundError as e:
        print(colored(f"Config Error: {e}", "red"))
        print(
            colored("Make sure coordinator has generated config files first", "yellow")
        )
        sys.exit(1)
