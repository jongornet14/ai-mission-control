...
class TorchGymWrapper:
    ...
    def reset(self):
        obs = self.env.reset()  # Gym environments now return (obs, info) for reset()
    ...
    def step(self, action):
        ...
        obs, reward, done, truncated, info = universal_gym_step(self.env, action)

        return (
            torch.FloatTensor(obs).to(self.device),
            torch.tensor(reward, dtype=torch.float32, device=self.device),
            torch.tensor(
                done or truncated, dtype=torch.float32, device=self.device
            ),  # Use 'done or truncated' for episode end
            info,
        )
...
class GymEnvironmentWrapper:
    ...
    def create(self):
        ...
        if self.max_episode_steps is not None:
            ...
            if (
                ...
                print(
                    f"Warning: Custom max_episode_steps ({self.max_episode_steps}) is less than environment's default ({env.spec.max_episode_steps}). Using custom."
                )
                env._max_episode_steps = self.max_episode_steps
    ...
    def _init_obs_normalization(self, env):
        ...
        self.obs_std = (
            np.std(obs_samples, axis=0) + 1e-8
        )  # Small epsilon to avoid division by zero

        print(
            f"Observation normalization initialized: mean={self.obs_mean}, std={self.obs_std}"
        )
