...
def initialize_bayesian_optimizer(shared_dir: str, config_loader: ConfigLoader) -> BayesianOptimizationManager:
    """
...
def generate_worker_configs(base_config: ConfigLoader, optimizer: BayesianOptimizationManager, 
                          num_workers: int, shared_dir: str) -> list:
    """
    ...
    try:
        ...
        for worker_id in range(num_workers):
            ...
            if 'learning_rate' in optimized_params:
                worker_config.set('algorithm.learning_rate', optimized_params['learning_rate'])
            if 'batch_size' in optimized_params:
                worker_config.set('algorithm.batch_size', int(optimized_params['batch_size']))
            if 'gamma' in optimized_params:
                worker_config.set('algorithm.gamma', optimized_params['gamma'])
            if 'entropy_coef' in optimized_params:
                worker_config.set('algorithm.entropy_coef', optimized_params['entropy_coef'])
            
            # Update architecture parameters if they exist
            if 'hidden_layer_1' in optimized_params:
                worker_config.set('algorithm.hidden_layer_1', int(optimized_params['hidden_layer_1']))
            if 'hidden_layer_2' in optimized_params:
                worker_config.set('algorithm.hidden_layer_2', int(optimized_params['hidden_layer_2']))
            if 'num_layers' in optimized_params:
                worker_config.set('algorithm.num_layers', int(optimized_params['num_layers']))
            
            # Add worker-specific metadata
            worker_config.set('worker.worker_id', worker_id)
            worker_config.set('worker.config_generated_at', optimized_params.get('suggested_at'))
...
def update_worker_performance(optimizer: BayesianOptimizationManager, shared_dir: str) -> bool:
    """
            ...
            try:
                ...
                if total_episodes >= 20:  # Minimum episodes for reliable performance
                    ...
                    if config_file.exists():
                        ...
                        optimization_params = worker_config.get('worker', {}).get('optimization_params', {})
                        ...
                        if optimization_params:
                            ...
                            updated_any = True
                            print(f"Updated performance for worker {worker_id}: {avg_reward:.2f}")
...
def handle_coordinator_error(error: Exception, shared_dir: str) -> None:
    """
    Handle coordinator errors by creating error files for monitoring
    ...
    try:
        ...
        print(f"Failed to log coordinator error: {log_error}")
...
class EnhancedBayesianCoordinator:
    ...
    def __init__(self, shared_dir: str, config_path: str, num_workers: int = None, check_interval: int = 30):
        """
        ...
        try:
            ...
            # Initialize Bayesian Optimizer
            self.optimizer = initialize_bayesian_optimizer(str(shared_dir), self.base_config)
    ...
    def run(self):
        ...
        try:
            while not self.should_terminate():
                ...
                # Update Bayesian Optimizer with worker performance
                updated = update_worker_performance(self.optimizer, str(self.shared_dir))
                ...
                if updated:
                    ...
                    if self.sync_count % 5 == 0:  # Every 5 syncs
                        print(f"Regenerating worker configs with updated optimization...")
                        self.worker_configs = generate_worker_configs(
                            self.base_config, self.optimizer, self.num_workers, str(self.shared_dir)
                        )
                
                # Regular coordination logic (find best worker, copy model, etc.)
                # ... (your existing coordination logic goes here)
...
def main():
    ...
    parser.add_argument('--num_workers', type=int, help='Number of workers (overrides config)')
