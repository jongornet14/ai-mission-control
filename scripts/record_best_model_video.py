...
from environments.gym_wrapper import GymEnvironmentWrapper # Adjust if your wrapper is elsewhere

def record_video(model_path, env_name, video_dir, num_episodes=1, max_episode_steps=None, device='cpu'):
    """
    ...
    dummy_env = GymEnvironmentWrapper(env_name, normalize_observations=False) # normalization should match training
    ...
    for i in range(num_episodes):
        ...
        env = GymEnvironmentWrapper(env_name, render_mode='rgb_array', max_episode_steps=max_episode_steps)
        # Wrap with RecordVideo. Note: This requires ffmpeg installed in the container
        env = gym.wrappers.RecordVideo(env, video_folder=str(video_filepath.parent), name_prefix=f"episode_{i+1}_{Path(model_path).stem}", disable_logger=True)
        ...
        with torch.no_grad():
            while not done and not truncated:
                action, _, _ = policy(torch.as_tensor(obs, dtype=torch.float32, device=device).unsqueeze(0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_dir', type=str, default='/workspace/videos',
