...
class CrazyLogger:
    ...
    def log_episode(self, **kwargs):
        ...
        if self.step_data:
            recent_steps = [
                s for s in self.step_data if s["episode"] == self.current_episode
            ]
            if recent_steps:
    ...
    def log_model_state(self, model, optimizer=None, loss=None, save_weights=True):
        ...
        if save_weights:
            # Save model weights
            model_path = (
                self.log_dir / "models" / f"model_episode_{self.current_episode}.pt"
            )
            torch.save(
                {
    ...
    def log_activations(self, model, input_data, layer_names=None):
        ...
        # Save activations
        activation_path = (
            self.log_dir / "activations" / f"activations_step_{self.global_step}.pkl"
        )
        ...
        for name, activation in activations.items():
            self.tb_writer.add_histogram(
                f"activations/{name}", activation, self.global_step
            )
            self.tb_writer.add_scalar(
                f"activations/{name}_mean", np.mean(activation), self.global_step
            )
            self.tb_writer.add_scalar(
                f"activations/{name}_std", np.std(activation), self.global_step
            )
    ...
    def log_video(self, frames, name):
        ...
        try:
            ...
            if frames and len(frames) > 0:
                ...
                video_tensor = torch.from_numpy(video_tensor).unsqueeze(
                    0
                )  # Add batch dimension
    ...
    def calculate_episode_stats(self, episode_steps):
        ...
        for step in episode_steps:
            for key, value in step.items():
                if isinstance(value, (int, float, np.number)) and not key.startswith(
                    "system_"
                ):
    ...
    def save_hyperparams(self):
        ...
        if self.hyperparams_history:
            with open(
                self.log_dir / "hyperparams" / "hyperparams_history.json", "w"
            ) as f:
    ...
    def save_system_metrics(self):
        ...
        for step_data in self.step_data[-1000:]:  # Last 1000 steps
            system_entry = {
                k: v for k, v in step_data.items() if k.startswith("system_")
            }
            if system_entry:
        ...
        if system_data:
            ...
            system_df.to_csv(
                self.log_dir / "system" / "system_metrics.csv", index=False
            )
