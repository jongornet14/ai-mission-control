...
class TestComputationCapabilities:
    ...
    def test_batch_environment_creation(self):
        ...
        # Check if sessions are unique or reused
        unique_sessions = set(session_ids)
        
        if len(unique_sessions) == 1:
            # Service reuses the same session for same environment type
            logger.info("ℹ️ Service reuses session IDs for same environment type")
            logger.info(f"✅ Batch environment creation working (shared session: {list(unique_sessions)[0]})")
        else:
            # Service creates unique sessions
            assert len(unique_sessions) == 5
            logger.info("✅ Batch environment creation working (unique sessions)")
...
class TestPerformanceAndReliability:
    ...
    def test_concurrent_environment_creation(self):
        ...
        # Check session IDs (may be same or unique depending on implementation)
        session_ids = [r["session_id"] for r in successful]
        unique_sessions = set(session_ids)
        
        if len(unique_sessions) == 1:
            logger.info("✅ Concurrent environment creation working (shared sessions)")
        else:
            logger.info(f"✅ Concurrent environment creation working ({len(unique_sessions)} unique sessions)")
...
class TestCompleteWorkflow:
    ...
    def test_simple_rl_workflow(self):
        ...
        # Check if service uses unique or shared sessions
        unique_batch_sessions = set(batch_sessions)
        if len(unique_batch_sessions) == 1:
            logger.info("✅ Service capacity: Can handle batch creation (shared session model)")
        else:
            logger.info(f"✅ Service capacity: Can handle batch creation ({len(unique_batch_sessions)} unique sessions)")
