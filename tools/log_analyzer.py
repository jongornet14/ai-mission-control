...
class LogAnalyzer:
    ...
    def find_log_files(self, component: Optional[str] = None, worker_id: Optional[int] = None) -> List[Path]:
    ...
    def load_logs(self, files: List[Path], since: Optional[datetime] = None) -> List[Dict]:
                ...
                with open(file_path, 'r') as f:
                    for line in f:
                        try:
                            ...
                            if since:
                                entry_time = datetime.fromisoformat(entry['iso_timestamp'])
                                if entry_time < since:
                                    continue
    ...
    def filter_by_event_type(self, entries: List[Dict], event_types: List[str]) -> List[Dict]:
        ...
        for entry in episode_entries:
            ...
            if 'episode' in entry and 'total_reward' in entry:
                ...
                worker_progress[worker_id]['latest_episode'] = max(
                    worker_progress[worker_id]['latest_episode'],
                    entry['episode']
                )
    ...
    def get_performance_metrics(self, entries: List[Dict]) -> Dict:
        ...
        perf_entries = [e for e in entries if 'system_info' in e or 'performance_snapshot' in e]
        ...
        for entry in perf_entries:
            perf_data = entry.get('system_info') or entry.get('performance_snapshot', {})
...
def main():
    ...
    parser.add_argument("--since", help="Show logs since time (e.g., '2h', '30m', '1d')")
    ...
    parser.add_argument("--hyperopt", action="store_true", help="Show hyperparameter optimization progress")
    ...
    print_colored(f"Loaded {len(entries)} log entries from {len(log_files)} files", "green")
    ...
    if args.training_progress:
        ...
        for worker_id, worker_data in progress.items():
            if worker_data['episodes']:
                ...
                recent_reward = worker_data['rewards'][-1] if worker_data['rewards'] else 0
    ...
    if args.performance:
        ...
        if cpu['samples'] > 0:
            print(f"CPU Usage: Avg {cpu['avg']:.1f}%, Max {cpu['max']:.1f}% ({cpu['samples']} samples)")
        
        memory = perf_metrics['memory_usage']
        if memory['samples'] > 0:
            print(f"Memory Usage: Avg {memory['avg']:.1f}%, Max {memory['max']:.1f}% ({memory['samples']} samples)")
        
        gpu_usage = perf_metrics['gpu_usage']
        if gpu_usage:
            print("GPU Usage:")
            for gpu in gpu_usage[-5:]:  # Show recent GPU usage
                print(f"  GPU {gpu['gpu_id']}: {gpu['utilization']:.1f}% util, {gpu['memory_percent']:.1f}% memory")
    ...
    if args.summary or not any([args.errors_only, args.training_progress, args.performance, args.hyperopt]):
        error_summary = analyzer.get_error_summary(entries)
        progress = analyzer.get_training_progress(entries)
        perf_metrics = analyzer.get_performance_metrics(entries)
        hyperopt = analyzer.get_hyperopt_progress(entries)
        ...
        if entries:
            ...
            print(f"Time Range: {start_time.strftime('%Y-%m-%d %H:%M:%S')} to {end_time.strftime('%H:%M:%S')}")
            print(f"Duration: {duration}")
        
        # Components and workers
        components = set(e.get('component') for e in entries if e.get('component'))
        workers = set(e.get('worker_id') for e in entries if e.get('worker_id') is not None)
